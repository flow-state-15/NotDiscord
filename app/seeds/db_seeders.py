...
def seed_server():
    ...
    # TODO update to a more permanent source
    server_icons = [
    ...
    for _ in range(0, 30):
        user = fake.user_name()
        server_name = (
            f"{user}'s {server_adjectives[randint(0, len(server_adjectives)-1)]}"
        )
        # default icon to a blank string in case one is not given
        new_icon = ""
        # 30% chance of no icon given to make it more realistic
...
def seed_user_server():
    ...
    new_user_server = User_Server(server_id=1, user_id=2)
    ...
    for server in range(1, total_servers + 1):
        ...
        for _ in range(1, 15):
            ...
            while user in users:
                user = randint(2, total_users)
            new_user_server = User_Server(server_id=server, user_id=user)
            db.session.add(new_user_server)
            users.append(user)
    db.session.commit()
...
def seed_friends():
    ...
    for _ in range(2, total_users - 1):
        ...
        new_user_server = Friend(
            sender_user_id=pair[0], rec_user_id=pair[1], accepted=True
        )
        db.session.add(new_user_server)
    db.session.commit()
...
def seed_channel():
    ...
    for channel in demo_channels:
        demo_channel = Channel(name=channel, server_id=1)
    ...
    for i in range(2, total_servers + 1):
        ...
        if i % 2 == 0:
            ...
            main_channel = "lounge"
        new_channel = Channel(name=main_channel, server_id=i)
        db.session.add(new_channel)
        for _ in range(1, channels_per_server + 1):
            while True:
                channel_name = random_channel_names[
                    randint(1, len(random_channel_names) - 1)
                ]
                if channel_name not in current_channels:
                    current_channels.append(channel_name)
                    new_channel = Channel(name=channel_name, server_id=i)
                    break
            db.session.add(new_channel)
    db.session.commit()
